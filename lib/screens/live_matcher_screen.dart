...
class _LiveMatcherScreenState extends State<LiveMatcherScreen> {
      ...
      if (image.format.group == ImageFormatGroup.yuv420) {
        ...
        for (int dy = -halfGrid; dy <= halfGrid; dy++) {
          ...
          for (int dx = -halfGrid; dx <= halfGrid; dx++) {
            ...
            final int uvIndex =
                (py ~/ 2) * uvRowStride + (px ~/ 2) * uvPixelStride;
            if (uvIndex >= uPlane.bytes.length ||
  ...
  Map<String, dynamic> _matchAgainstSeason() {
    ...
    if (_latestScan != null) {
      ...
      final occasionPalette =
          palettes[_selectedOccasion] ??
          palettes['casual'] ??
          palettes['office']!;
      activePaletteColors = [
        occasionPalette.primaryColor,
        occasionPalette.secondaryColor,
        occasionPalette.accentColor,
      ];
    } else {
      // Use occasion-aware seasonal fallback colors
      final key = '$_selectedSeason|$_selectedOccasion';
      activePaletteColors =
          _fallbackSeasonColors[key] ?? ['#FFFFFF', '#888888', '#000000'];
    }
    ...
    return {'score': score, 'target_hex': matchedTargetHex};
  }
